...
#   03_get_codes.py
#
#  Defines codes of Receiver output signals 
#
...
# Collects input data for 'duration' period (duration is in sec)
def aquire_binary(pin, duration):
    t0      = time()
    result = []
...
def handle_signal(pin, reading_time=150):
    data = aquire_binary(pin, reading_time/1000.0)

    if len(data) < reading_time:
        return None

    rate = len(data) / (reading_time / 1000.0) # acquisition rate (s)

    pulses = []
    i_break  = 0

    # Detect run lengths using the acquisition rate to turn the timesin to microseconds
...
def on_ir_receive(pinNo, bouncetime=150):       # 150 ms - ~615 times getting data
...
if __name__ == "__main__":
    ...
    try:
        ...
        while True:
            ...
            edge_detected = GPIO.wait_for_edge(ir_pin, GPIO.FALLING)

            if edge_detected:
                code = handle_signal(ir_pin)
